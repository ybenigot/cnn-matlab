fprintf('hyperparameters TEST MIN 3\n');
...
ei.activation_functions = ['relu    ';'identity'];
ei.cost_function = 'cross_entropy_cost'; % possibilities are square_cost and cross_entropy_cost 

ei.layer_type= [convolve;pooling];
ei.pool_types= ['N/A';'max'];
% sizes of all hidden layers and the output layer ; number of features for convolutional layer
ei.layer_sizes = [2*20*20,ei.output_dim];
ei.dropout =[0,0];

% hyperparameters for convulutional layer(s)
ei.image_size = [ei.input_pixels,20];				% pixel size of image for each layer
ei.filter_sizes = [9,-1];							% filter size for convolutional layers
ei.numFilters = [2,2];
ei.pool_dim = [-1,2];
