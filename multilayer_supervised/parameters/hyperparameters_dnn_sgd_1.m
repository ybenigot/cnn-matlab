fprintf('hyperparameters DNN1 SGD\n');
...
ei.activation_functions = ['relu';'relu';'softmax'];
ei.cost_function = 'cross_entropy_cost'; % possibilities are square_cost and cross_entropy_cost 

ei.layer_type= [connected,connected,connected];
% sizes of all hidden layers and the output layer ; number of features for convolutional layer
ei.layer_sizes = [150, 60, ei.output_dim];
ei.image_size = [0,0,0];				% pixel size of image for each layer

ei.filter_sizes = [-1,-1,-1];							% filter size for convolutional layers
ei.numFilters = [-1,-1,-1];
ei.pool_dim = [-1,-1,-1];
