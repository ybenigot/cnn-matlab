fprintf('hyperparameters TEST MIN 4\n');
...
ei.activation_functions = ['tanh    ';'identity';'softmax '];
ei.cost_function = 'cross_entropy_cost'; % possibilities are square_cost and cross_entropy_cost 

ei.layer_type= [convolve,pooling,connected];
ei.pool_types= ['N/A';'max';'N/A'];
% sizes of all hidden layers and the output layer ; number of features for convolutional layer
ei.layer_sizes = [2*20*20, 2*10*10, ei.output_dim];
ei.image_size = [ei.input_pixels,20,-1];				% pixel size of image for each layer

ei.filter_sizes = [9,-1,-1];							% filter size for convolutional layers
ei.numFilters = [2,2,-1];
ei.pool_dim = [-1,2,-1];
